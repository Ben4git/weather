...
#        lines = f.readlines()
    ...
    class1=np.array([line[:6] for line in lines if "good" in line[-1]], dtype=np.float) 
    class2=np.array([line[:6] for line in lines if "good" not in line[-1]], dtype=np.float)
